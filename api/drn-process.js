...
/**
 * Debit Note (DRN) end-to-end processor.
 *
 * Expects POST body:
 * {
 *   invoice_id: "...",
 *   invoice_number: "...",
 *   ref_invoice: "existingFiscalisedInvoiceIdentifier",   // REQUIRED
 *   reason: "Reason for debit note",                      // REQUIRED
 *   invoice_data: { ... Zoho invoice JSON or object ... }
 * }
 *
 * This reuses the same AES/RSA/token/encrypt/transmit flow as mra-process.js.
 */

export const config = { runtime: "nodejs" }; // ensure Node.js runtime on Vercel
...
export default async function handler(req, res) {
  try {
    ...
    const { invoice_id, invoice_number, ref_invoice, reason, invoice_data } = req.body || {};
    ...
    if (!ref_invoice || typeof ref_invoice !== "string" || ref_invoice.trim() === "") {
      return res.status(400).json({
        status: "ERROR",
        message: "Missing required field: ref_invoice (the invoiceIdentifier of an existing fiscalised invoice)"
      });
    }
    ...
    if (!reason || typeof reason !== "string" || reason.trim() === "") {
      return res.status(400).json({
        status: "ERROR",
        message: "Missing required field: reason (reasonStated for DRN)"
      });
    }
    ...
    // --- Date/time helper (same as in mra-process) ---
    ...
    if (rawLineItems.length === 0) {
      return res.status(400).json({
        status: "ERROR",
        message: "Debit note must contain at least one line item."
      });
    }
    ...
    // Build DRN invoice (note invoiceTypeDesc = "DRN" and invoiceRefIdentifier = ref_invoice)
    const mraInvoice = {
      ...
      invoiceRefIdentifier: String(ref_invoice),
      previousNoteHash: previousNoteHash,
      reasonStated: String(reason),
    ...
    const tokenUrl = process.env.MRA_TOKEN_URL || "https://vfisc.mra.mu/einvoice-token-service/token-api/generate-token";
    ...
    // --- Step 6: Transmit ---
    const transmitUrl = process.env.MRA_TRANSMIT_URL || "https://vfisc.mra.mu/realtime/invoice/transmit";
    const pad = (n) => String(n).padStart(2, "0");
    const now = new Date();
    ...
    // TODO: persist original Zoho, mraInvoice, encryptedInvoice, transmitData
